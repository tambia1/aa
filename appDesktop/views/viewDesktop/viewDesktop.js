...
app.appDesktop.ViewDesktop.prototype.initViews = function () {
	...
	this.paging = new spa.Paging('view_desktop__paging', '', false, null, null, null, null);
	this.addItem(this.paging);

	for (let i = 0; i < pages.length; i++) {
		let itemPage = new spa.Item('<div class="view_desktop__item" data-index="' + i + '"></div>');
		...
		for (let j = 0; j < pages[i].length; j++) {
			let itemUiButtonApp = new app.appDesktop.UiButtonApp(
				...
				() => {
					...
					} else if (this.isButtonsShaking == false && pages[i][j].appView != null) {
...
app.appDesktop.ViewDesktop.prototype.shakeButtons = function (pages, isButtonsShaking) {
